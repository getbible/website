---
title: "Query API v3"
description: "Reference syntax, chapter-grouped responses, canonical URLs, errors and working examples for Query v3."
---

Query v3 resolves Scripture references against the Bible v3 data tree. Send the translation and reference in the path; receive the selected verses grouped by translation, book and chapter.

**Data route:** `GET https://query.getbible.net/v3/{translation}/{reference}`

[Live documentation](https://query.getbible.net/v3/) · [OpenAPI JSON](https://query.getbible.net/v3/openapi.json) · [Query overview](/api/query/) · [Support](https://git.vdm.dev/getBible/support)
...
| --- | --- | --- |
| API version | Path | `v3`; select it explicitly in production integrations |
...
curl --fail-with-body --compressed \
  'https://query.getbible.net/v3/kjv/John%203:16,18-21;Romans%208:1-4'

# A complete short chapter
curl --fail-with-body --compressed \
  'https://query.getbible.net/v3/kjv/Psalm%2023'
```
...
The six translation metadata fields are copied as available from the validated source; optional omissions and accepted empty values remain unchanged. `name` belongs to the chapter, while the translation name is `translation`. The full translation history, description, distribution terms and source details are available separately from [the v3 catalogue](https://api.getbible.net/v3/translations.json).
...
Version 3 retains optional `paragraph`, `tokens` and `spans` from the source verse, together with any other source verse fields. A verse without these fields remains without them. Word positions are one-based inclusive display-word positions; token ranges in spans are zero-based inclusive indexes into the verse token array. Do not assume a token equals exactly one displayed word.

The live John 3:16 response includes a paragraph-start flag and lexical records. One actual token from that response is:
...
The source chapter's `editorial` array is deliberately omitted from assembled query results. Request [the static chapter](https://api.getbible.net/v3/kjv/43/3.json) when rendering its complete headings and paragraph ranges. See [Bible v3](/api/bible/v3/) for full token, span and editorial semantics.

## Canonical URLs and defaults

| Request | Behavior |
| --- | --- |
| `/v3/kjv/John3:16` | Returns the requested selection |
| `/v3/John3:16` | Valid short reference; `301` to `/v3/kjv/John3:16` |
...
| `/v3/kjv` | `404 missing_reference` |
| `/v3/unknowntranslation/John3:16` | `404 translation_not_found` |
...
curl --fail-with-body --location --compressed \
  'https://query.getbible.net/v3/John3:16'

# Inspect the documented error for a missing reference
curl --include \
  'https://query.getbible.net/v3/kjv'
```

The documentation page is `/v3/` with its trailing slash. Use its documented URL when linking to the human guide; a scripture route without the required reference is a different request.

## Complete JavaScript example

```javascript
async function getScripture(reference, translation = 'kjv') {
  const url = `https://query.getbible.net/v3/${encodeURIComponent(translation)}/${encodeURIComponent(reference)}`;
...
curl --fail-with-body --compressed \
  ...
  --output getbible-query-v3-openapi.json
```
...
This guide follows the [live v3 contract](https://query.getbible.net/v3/openapi.json), [service documentation](https://query.getbible.net/v3/), [runtime policy](https://github.com/getbible/api/blob/main/docs/RUNTIME_ENDPOINTS.md), [reference examples](https://github.com/getbible/librarian/blob/master/docs/USAGE.md), and [query limit configuration](https://github.com/getbible/api/blob/main/src/apps/query/getbible_query_api/config.py). Report problems at [GetBible support](https://git.vdm.dev/getBible/support), including the request URL, HTTP status and `X-Request-ID` header.
