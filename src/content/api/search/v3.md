---
title: "Search API v3"
description: "Complete full-text search routes, filters, matching rules, pagination, result schemas and cURL recipes for v3."
---

Search v3 finds Scripture in the Bible v3 data tree. It accepts words, phrases and structured filters through GET or POST, returning both an ordered match list and the matching verses grouped by chapter. If the input resolves as a Scripture reference, it returns that passage in the same outer envelope.

**Canonical route:** `https://search.getbible.net/v3/{translation}/{search}`

[Live documentation](https://search.getbible.net/v3/) · [OpenAPI JSON](https://search.getbible.net/v3/openapi.json) · [Search overview](/api/search/) · [Support](https://git.vdm.dev/getBible/support)
...
| --- | --- | --- |
| GET | `/v3/{translation}/{search}` | Search text in the path; filters in query parameters |
| GET | `/v3/{translation}?q=...` | Translation in the path, search text and filters in query parameters |
| GET | `/v3?q=...&translation=...` | Search text and translation in query parameters |
| POST | `/v3/{translation}/{search}` | Path text; filters in query parameters and/or JSON |
| POST | `/v3/{translation}` | Search text in query parameters and/or JSON |
| POST | `/v3` | Translation, search text and filters in query parameters and/or JSON |
...
`translation` defaults to `kjv` when omitted on a route accepting it as a parameter. An explicitly supplied unknown translation returns `404`. Use an abbreviation from [the v3 translation catalogue](https://api.getbible.net/v3/translations.json).
...
```bash
curl --fail-with-body --compressed --get \
  'https://search.getbible.net/v3/cus' \
...
### v3 verse representation

Version 3 preserves optional source `paragraph`, `tokens` and `spans`, as well as other source verse fields and nested values. It does not invent metadata for verses that lack it. Each token has reading-order text and inclusive one-based display-word positions. Spans carry zero-based inclusive indexes into `tokens` and one-based inclusive display-word ranges; 0 in a display range means unlocated.

Chapter-level `editorial` is deliberately omitted from search-assembled chapters, including reference-kind responses. If the app needs complete headings and paragraph ranges around a match, fetch that chapter from [Bible v3](/api/bible/v3/). Preserve the whole verse dictionary in storage or downstream responses so lexical attributes are retained.
...
async function searchScripture(text, offset = 0) {
  ...
  const response = await fetch(`https://search.getbible.net/v3/kjv?${parameters}`);
...
Short forms such as `/v3/faith%20hope` redirect to the default translation. Unversioned aliases follow the domain's configured default endpoint. GET aliases use `301`; POST aliases use `308` to preserve method and body. Prefer an explicit canonical route when integrating.
...
curl --fail-with-body --compressed \
  ...
  --output getbible-search-v3-openapi.json
```
...
This guide follows the [live v3 OpenAPI](https://search.getbible.net/v3/openapi.json), [live service guide](https://search.getbible.net/v3/), [runtime implementation contract](https://github.com/getbible/api/blob/main/docs/RUNTIME_ENDPOINTS.md), [librarian search semantics](https://github.com/getbible/librarian/blob/master/docs/SEARCH.md), and [compact metadata contract](https://github.com/getbible/librarian/blob/master/docs/TRANSLATION_METADATA.md). Ask for help at [GetBible support](https://git.vdm.dev/getBible/support), including the URL, status and `X-Request-ID` response header without any access token.
